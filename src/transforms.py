...
def get_coord_grid(img, xscale=np.pi, yscale=np.pi):
    x = np.linspace(-xscale, xscale, img.shape[1])
    y = np.linspace(-yscale, yscale, img.shape[0])
...
def change_coords(img, new_coords, blend=True):
    x = new_coords.real
    y = new_coords.imag
